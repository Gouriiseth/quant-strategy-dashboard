...
class SectorRotationStrategy(BaseStrategy):
    ...
    DESCRIPTION = (
        "Signal-triggered rebalancing | RS-rank weighting | "
        "Breakout quadrant | Decoupled vol-target + stop-loss | "
        "3-phase market detector (Bull/Sideways/Bear) | Trend-consistency stock scoring. "
    ...
    def render_sidebar(self):
        ...
        st.sidebar.markdown("---")
        st.sidebar.markdown("**Phase Detector**")
        self.bear_gap_threshold = st.sidebar.slider(
            "Bear gap threshold %", -12, -3, -5,
            help="Gap below 200DMA to classify market as Bear phase."
        ) / 100
        self.sideways_gap_pct = st.sidebar.slider(
            "Sideways band %", 3, 10, 5,
            help="±% from 200DMA to classify market as Sideways."
        ) / 100

        st.sidebar.markdown("---")
        st.sidebar.markdown("**Signal-Triggered Rebalancing**")
        ...
        self.stop_loss_pct = st.sidebar.slider(
            ...
            help="Per-sector trailing stop-loss threshold."
        ) / 100
        self.vol_target = st.sidebar.slider(
            "Portfolio Vol Target %", 8, 20, 14,
            help="Target annualised volatility for position sizing."
        ) / 100
    ...
    def run(self):
        with st.spinner("Running Sector Rotation (~3-4 min)..."):
        ...
        c5.metric("Signal Rebals",   f"{sr_ct}",
                  help="Extra rebalances fired by quadrant-change signals")
        ...
        st.caption("Red shaded = Bear phase (fires when Nifty is below bear gap threshold)")
        ...
        fig2.add_trace(go.Scatter(
            ...
            fill="tozeroy", name="Sector Rotation",
        ...
        if len(ys) > 0:
            ...
            fig3.add_trace(go.Bar(
                ...
                name="Sector Rotation", marker_color=clrs, opacity=0.90))
            fig3.add_trace(go.Bar(
        ...
        c3.metric("Signal Rebals",   f"{sr_ct}",
                  help="Extra rebalances from quadrant changes")
...
| Component | Problem it Solves | Solution |
|---|---|---|
| **Signal-Triggered Rebalancing** | Calendar rebal held weakening sectors for weeks | Rebalances whenever a sector's RS quadrant changes |
| **RS-Rank Weighting** | Equal/quality weighting bought late into extended sectors | Higher RS rank = more capital allocation |
| **Breakout Quadrant Entry** | Only Leading/Improving entries meant lagging signals | Breakout quadrant (RS just crossed 100) enables early entry |
| **Vol-Freeze After Stop** | Stop-loss + vol-target firing together forced near-cash | 5-day vol-freeze after stop; exposure floor at 70% |
| **3-Phase Market Detector** | Binary bull/bear missed shallow corrections | Bull / Sideways / Bear based on gap to 200DMA |
| **TF Agreement by Phase** | Same TF threshold in all phases → noise in Sideways | 3/3 in Sideways, 1/3 in Bear, 2/3 in Bull |
| **Trend Consistency Score** | Stock scoring missed trend integrity | +% days above 20DMA added to sector stock score |
| **Min Sector Hold** | Signal rebal could whipsaw freshly entered sectors | {self.min_sector_hold}-day protection on signal-triggered exits |

### Current Parameter Settings
| Parameter | Value | Effect |
|---|---|---|
| Bear gap threshold | {self.bear_gap_threshold*100:.0f}% | Gap below 200DMA to trigger Bear phase |
| Sideways band | ±{self.sideways_gap_pct*100:.0f}% | Band around 200DMA for Sideways classification |
| Vol target | {self.vol_target*100:.0f}% | Target annualised portfolio volatility |
| Stop-loss | {self.stop_loss_pct*100:.0f}% | Per-sector trailing stop threshold |
| Min sector hold | {self.min_sector_hold}d | Minimum hold after entry before signal exit allowed |

### Tuning Guide
1. **Low alpha (<5%)**: Lower `min_signal_days` to 5, lower `bear_gap_threshold` to -4%
